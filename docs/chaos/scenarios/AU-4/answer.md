...
## 채점 앵커 — **v3 (2026-07-27 · 회차 1부터 적용 — 구 07-27 라운드는 폐기)**

> **회차 재정의 (2026-07-28)**: 07-27 라운드는 v1 앵커가 사실과 달라(3s timeout) 채점
> 불가였고, 이를 **파일럿(폐기)**으로 돌린다. v3 앵커는 그 파일럿 실측으로 07-27에 작성돼
> **07-28 재실행 주입보다 앞서므로**, 재실행을 유효 **회차 1**로 하고 v3로 채점한다(§8.2
> "정정은 채록 전" 충족). 파일럿의 trace 대조 데이터는 재실행과 모양이 동일해 근거로 보존한다.
...
| 근거 경로 30 | ⓐ auth 호출 client span의 error를 근거로 쓰되 **실패 방식을 구별**(즉시 거절 vs 3s timeout) + ⓑ **fallback 로그**(`사용자 목록 조회 실패`) + ⓒ 작성자 익명 전환 | 20: 3종 중 2종 / 또는 **동등한 대체 경로로 도달**(예: auth 메트릭 시계열 소멸을 대조군과 함께 제시) | 10: 작성자 익명 또는 500만 인용 | 근거 없음 |
...
> **v4 정정 (2026-07-28) — 만점 요건에서 `baseline 대비 auth 서버 span 출현/부재 대조`를 뺐다.**
>
> **사유는 채널 사실이다**: v0 입력은 traceId 1개뿐이라 baseline 트레이스를 받지 않는다.
> 구조적으로 달성 불가한 요건이었고, 회차 1에서 이 항목으로 −10점이 나갔다.
> **IN-2 v2가 같은 사유로 "baseline producer span 대조"를 제거한 선례를 따른다.**
> (에이전트 답안에서 착안한 정정이 아니라 입력 설계에서 나온 정정이므로 §8.2 오염과 무관하다.)
>
> **캐시 만료 입증은 근본 원인 항목이 이미 요구한다** — 근거 경로에서 이중으로 요구할 필요가 없다.
> AU-2와의 구별도 ⓒ(익명 전환)가 담당한다: AU-2 갈래 A는 캐시 히트라 **작성자가 실명 유지**다.
>
> **ⓑ fallback 로그를 복원했다.** v3에서 "관측 불가"를 이유로 뺐으나 **그 전제가 깨졌다** —
> `ExternalUserApiClient.java:130`이 `log.error("사용자 목록 조회 실패: userIds={}")`이고,
> rca-agent의 `errorWarnQuery`는 `{service_name=~"..."} |~ "ERROR|WARN"` **라인 필터라
> traceId가 필요 없다.** [NF-09](../../../../../yogurtte-rca-agent/docs/findings/nf-09-user-fallback-no-traceid.md)의
> `traceId=NONE`은 `traceIdQuery`만 막을 뿐이다. 시간창 안에 있으면 수집된다.
>
> 다만 **traceId 연결이 없어 "이 요청의 로그"임은 시간으로만 추정**된다 — A-2(리액티브
> traceId 전파)의 값어치는 *도달 가능성*이 아니라 *귀속 정확도*에 있다.
>
> **적용 시점**: 회차 2 채록 전 박제이므로 §8.2 충족. 회차 1 점수(75)는 그대로 둔다.
...
| 항목 | 배점 | 파일럿 (v1, 폐기) | **회차 1 (v3, 갈래 A)** | 회차 2 (v3) |
|---|---|---|---|---|
| 근본 원인 정확도 | 40 | (40) | **30** | |
| 근거 시그널 경로 | 30 | (N/A) | **20** | |
| 오귀인 없음 | 20 | (20) | **20** | |
...
- **파일럿(폐기)**: v1 앵커 근거경로 요건("3s timeout", "fallback 로그")이 **실현되지 않는 것**
  이었다. 에이전트가 "타임아웃이 아니라 TCP RST"라고 **앵커를 정정** → v3의 근거. 점수 미집계.
- **회차 1 (재실행, 간결본 out 2,980)**: v3 갈래 A로 채점 75. 근본 30(부분점상 — auth 다운
  확정, **캐시 만료 발현 조건 미언급**) · 근거 20(부분점상 — 실패방식 구별·익명 전환 2종,
  baseline auth span 대조 없음) · 오귀 20 · 조치 5(auth 복구만, 캐시 TTL 없음). 파일럿 장황본은
  Redis GET으로 캐시 만료를 특정했으니 ~100 — **같은 v0 두 실행 25점 차, §8.1 N≥2 실증**.
- 평균 ± 최대편차: 산출 불가 (유효 채점 N=1) — 회차 2 필요.
- 자기 일치도(§8.2): 미실시

## 실행 결과

### 파일럿 (폐기) — 2026-07-27, 갈래 A (fallback 정상 저하)

> 앵커 발견 라운드. v1이 사실과 달라 채점 불가 → v3 도출의 근거. 점수 미집계, **아래
> trace 대조 데이터는 재실행 회차 1과 모양이 동일해 근거로 보존**. 유효 회차 1은 07-28 재실행.
...
### 회차 1 — 2026-07-28 재실행 (v3 갈래 A), 75/100

- 재주입 14:29Z~ · symptom traceId `6a676edbd07a4b966d03330e318ef61d` (66 spans, 174ms)
- 측정: in 56,325 / **out 2,980** tok · **$0.5209** · 58.2s (간결본)
- trace 모양은 파일럿과 동일 — `http get` client span이 23.55ms `STATUS_CODE_ERROR`(refused),
  앞에 redisGET 4건(캐시 미스). 스크린샷은 파일럿 것(img*.png) 재사용.
- 채점 75: 위 채점 표 참조. **캐시 만료 미식별**로 근본 30·조치 5(파일럿 장황본은 특정 → ~100).
