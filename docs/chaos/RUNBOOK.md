...
> 🔴 **회차 2부터 입력 모델과 루브릭이 바뀐다 (2026-07-28 확정, 주입 전).**
>
> | | 회차 1까지 (v0) | **회차 2~ (v1)** |
> |---|---|---|
> | 입력 | 채록 창 + **traceId 1개** | **자연어 질문 하나** (문안은 `anchors-v2.md`에 박제) |
> | 앵커 | `scenarios/<ID>/answer.md` | **[anchors-v2.md](anchors-v2.md)** (전 문항 SoT) |
> | 루브릭 | 아래 4항목 | **[루브릭 v3](../../../yogurtte-rca-agent/docs/scoring/rubric-v3.md) 6항목** — 원인 40 · 근거 25 · **탐색 15** · **영향 10** · 오귀인 5 · 조치 5 |
>
> **아래 절차와 §8.1·§8.2 규칙은 그대로 적용된다.** 바뀌는 것은 1번(입력 고정)의 내용물과
> 3번(채점)이 대조할 앵커 파일이다. 자연어 입력에서는 **질문 문안 자체가 입력 패키징**이므로
> 회차 간 문구를 동일하게 유지하는 것이 §8.2 준수의 핵심이다.
>
> **구 앵커(`answer.md`)와 아래 4항목 루브릭은 지우지 않는다** — 회차 1까지의 점수가 그 자로
> 매겨졌고, §8.2가 구버전 삭제를 금한다.
...
| 근거 시그널 경로 | 30 | **원인 확정에 충분한 근거 집합**(앵커가 정의)에 도달해 인과로 연결 — 신호 개수가 아니라 증거력 기준. 동일 정보를 담은 신호는 동치(로그 원문 ↔ span error 태그) |
...
- **충분 근거 원칙 (2026-07-27 신설)**: 근거 경로는 신호 개수가 아니라 **증거력**으로 채점한다. 앵커는 "이 문항의 원인을 확정하기에 충분한 근거 집합"을 동치 단위로 정의한다 — 같은 정보를 담은 신호는 동치이며(예: 로그의 "Data too long" 원문 ↔ INSERT span error 태그의 동일 원문), 어느 쪽으로 도달하든 만점이다. 장애가 로그만으로 확정 가능한 문항이면 로그만으로 만점. 충분 집합 밖의 신호를 추가로 모으지 않았다는 이유로 감점하지 않는다. 단 고유 정보를 담은(동치가 아닌) 신호는 별도 요건으로 남길 수 있고, 그 정보의 용도가 원인 확정이 아니라 영향 판정이면 근거 경로가 아닌 해당 항목의 요건으로 옮긴다. (AP-1 회차 1 채점자 판정의 일반화)
- ~~시간 분리~~ (**2026-07-27 폐지**): "채록과 채점 사이 최소 하루" 요건은 제거한다. 냉각의 실효가 앵커 문언 인용 대비 낮았고, 하루 지연이 채점 적체를 유발했다(채점 대장: AE-01~03 방치 — "채점 없는 조사는 baseline이 아니라 일화다"). 채점 재량 통제는 앵커 선박제 + 문언 인용 + 자기 일치도 검사가 담당한다. 여러 문항을 몰아 채점할 땐 문항 순서 섞기는 유지.- **자기 일치도 검사**: 동일한 RCA 출력을 이틀 이상 간격으로 2회 채점해 점수 차를 answer.md에 기록한다. **차이가 ±5를 넘으면 앵커가 모호한 것** — 앵커를 보강하고 재채점한다.
