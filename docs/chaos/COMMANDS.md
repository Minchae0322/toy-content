...
- 통째로 돌리려면 **`./chaos.sh <ID> run` 한 줄이면 된다** — 문항별 유지 시간만큼 자동으로
  기다린다(무인). Ctrl-C 시 자동 원복 trap. 단계마다 멈추려면 `CHAOS_CONFIRM=1 ./chaos.sh <ID> run`.

### `run`의 문항별 유지 시간 (자동)

`on → symptom`(발현·적재 대기)과 `symptom → off`(채록 후에도 전개가 계속되는 구간)를
문항마다 따로 잡는다. 값의 근거는 `scripts/chaos.sh`의 "문항별 유지 시간" 주석에 있다.

| 문항 | on→symptom | symptom→off | 왜 그 값인가 |
|---|---|---|---|
| CH-1 | 60s | **240s** | ④의 T1이 재시도를 소진하고 DLQ에 적재되기까지 ≈123s(4×30s + 3×1s). 총 다운 ≥5분(RUNBOOK §6) |
| CH-2 | 60s | — | 유지 자체는 `trigger`(30건×10초 ≈ 5분)가 담당. 이 60초는 **마지막 댓글분 lag가 스크레이프될 시간** |
| CH-3 | 60s | — | **다운은 `CH3_DOWN_SECONDS`(20s)로 고정**이고 `trigger`가 자동 원복까지 한다. 이 60초는 복구 **후** 대기 — 지연 트레이스가 Tempo에 적재돼야 `duration>10s` 검색이 잡힌다 |
| AU-1 | 660s | — | trigger 후 user 캐시 TTL 10분 경과해야 fallback이 보인다 (**문항 보류 중**) |
| AU-2 | 60s | — | **TTL 10분 안쪽**에서 채록 — 넘기면 캐시가 만료돼 AU-4 갈래가 된다(스크립트가 거부) |
| AU-3 | 60s | — | `inject`가 warm 트래픽 + `rollout status`까지 대기 |
| AU-4 | 660s | — | 캐시 TTL 10분 만료 + 여유 (RUNBOOK §6 "10분+ 유지") |
| IN-1 | 180s | — | 캐시 미스 직행·프레즌스 이상 관측용 |
| IN-2 | 90s | **90s** | ④의 T1 발행이 `max.block.ms`(기본 60s)를 소진하고 실패해야 유실이 성립 |
| IN-3 | — | — | **`run` 미지원** — `inject`(k6)가 블로킹이라 아래 2터미널 절차를 쓴다 |
| AP-1·2·3 | 60s | — | 인프라 무접촉·즉발 — 로그 전송 대기만 |

> ⚠️ **`symptom→off`가 있는 두 문항(CH-1·IN-2)에서 일찍 원복하면 문항이 조용히 무효가 된다.**
> CH-1은 재시도 중 Mongo가 살아나 첫 시도가 성공해 **CH-3 갈래로 뒤바뀌고**, IN-2는 60초 안에
> Kafka가 살아나 **발행이 성공해 유실이 안 난다.** 수동 단계로 돌릴 때도 이 대기를 지켜야 한다.

### 유지 시간이 곧 문항 정체성인 쌍 — 스크립트가 가드로 막는다

같은 주입인데 **얼마나 유지하느냐로 다른 문항이 되는** 쌍이 둘 있다. 잘못 잡으면 실패가 아니라
**다른 문항이 조용히 실행돼** 앵커와 안 맞는 채록이 남는다.

| 쌍 | 경계 | 짧으면 | 길면 | 가드 |
|---|---|---|---|---|
| CH-1 ↔ CH-3 | Mongo `serverSelectionTimeoutMS` **30초** | CH-3 (대기 중 복구 → 지연 흡수) | CH-1 (예외 → 재시도 → DLQ) | `CH3_DOWN_SECONDS ≥ 30`이면 거부 · CH-1은 총 다운 하한 300초 강제 |
| AU-2 ↔ AU-4 | user 캐시 TTL **10분** | AU-2 (캐시 히트 → content 무영향) | AU-4 (캐시 만료 → 익명 fallback) | `AU2_HOLD_PRE ≥ 600`이면 거부 |

값을 바꾸려면 환경변수로 덮어쓴다: `CH1_HOLD_POST=300 ./chaos.sh CH-1 run`
